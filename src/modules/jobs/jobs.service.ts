...
export class JobsService {
  ...
  async findAll(filterJobsDto: FilterJobsDto) {
    const { keyword, jobType, status, cityId, isRemote, isUrgent, companyId, industryId, countryId, page, limit } =
      filterJobsDto
    ...
    // Filter by industry
    if (industryId) {
      query.andWhere('job.industryId = :industryId', { industryId })
    }

    // Filter by country
    if (countryId) {
      query.andWhere('job.countryId = :countryId', { countryId })
    }
